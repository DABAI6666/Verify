...
def verification_loop_txt(model,task1_out,corpus,question,batch_size,top_k,evidence_type,class_type,model_mode,vl_model, run_dir,args,wte_model,tokenizer,stop_tokens_tensor,
...
############################################## testing loop #############################################
    ...
    if args.media=='txt':
        ...
        verificaiton_out=verification_loop_txt(model,task1_out,corpus,args.prompt,args.batch_size,args.top_k,args.evidence_type,args.class_type,'binary',vl_model, args.run_dir,
                                                      args,wte_model,tokenizer, stop_tokens_tensor, sub_tokens_tensor, None, None, device)
        ...
        # sub_tokens_tensor, rank_lm, logger, device):
        verificaiton_out=verification_loop_multimodal(model,task1_out,text_corpus,args.prompt,args.batch_size,args.top_k,
...
def get_args():
    ...
    parser.add_argument("--prompt", type=list, default=['Based on the provided evidence, objectively describe the information relevant to the claim'])
    ...
    parser.add_argument("--model_name", default='xtuner/llava-llama-3-8b') #{Open-Orca/Mistral-7B-OpenOrca, Salesforce/instructblip-flan-t5-xl , llava-hf/llava-1.5-7b-hf}
